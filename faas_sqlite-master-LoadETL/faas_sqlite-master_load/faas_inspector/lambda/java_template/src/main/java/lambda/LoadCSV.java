...
public class LoadCSV implements RequestHandler<Request, Response>
{
	...
	long cputime0 = ManagementFactory.getThreadMXBean().getThreadCpuTime(java.lang.Thread.currentThread().getId());
	 ...
	 try
	    ...
	    //creating putObjectRequest
            PutObjectRequest req = new PutObjectRequest(bucketname, "salespipeline.db", new File("/tmp/salespipeline.db"));
	    ...
	    s3Client.putObject(new PutObjectRequest(bucketname, "salespipeline.db", new File("/tmp/salespipeline.db")));
        }
        catch(Exception e)
        {
	...
	//getting the number of rows inserted 
	 int rows = Integer.parseInt(getRowsInserted());
	 ...
	 // Set return result in Response class, class is marshalled into JSON
        ...
        r.setClientid(request.getClientid());
        r.setFilename("salespipeline.db");
        r.setBucketname(request.getBucketname());
        r.setQuery(request.getQuery());
        r.setServiceoneturnaroundstarttime(request.getServiceoneturnaroundstarttime());
        return r;
    }
    public static String getRowsInserted(){
	String rows = "";
	setCurrentDirectory("/tmp");
	 try
        {            
